...
export default async function ParkPage() {
  ...
  const rows: RollRowView[] = roll.rows.map((r) => ({
    ...
    state: r.state,
    active: r.lot.active,
    currentRenter: r.current ? roll.renterNames.get(r.current.renterUserId) ?? "Renter" : null,
    ...
    nextRenter: r.next ? roll.renterNames.get(r.next.renterUserId) ?? "Renter" : null,
    nextFrom: r.next?.range?.start ?? null,
    pending: r.pending.map((p) => ({
      id: p.id,
      renter: roll.renterNames.get(p.renterUserId) ?? "Renter",
