...
export interface ReconcileInput {
  ...
  /**
   * The month that contains the takeover. In it the machine never says "late" —
   * the seller collected the first half and the roll is half-entered, so it is
...
export function reconcile(input: ReconcileInput): Finding[] {
  const out: Finding[] = [];
  const { lots, openClaims, month, cutoverMonth } = input;
  const inCutoverMonth = cutoverMonth != null && cutoverMonth === month;
  ...
  if (unbilled.length > 0 && !inCutoverMonth) {
 ...
 */
export function cutoverMonthNote(month: string, cutoverMonth: string | null): string | null {
