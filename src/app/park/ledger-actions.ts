...
import { feesFor, unbilledCostShares } from "./charge-edits";
...
// Whether anything more will ever bill for a household — the one read every
// "comes off their next bill" keys on.
...
// feesFor and unbilledCostShares live in ./charge-edits now — the run, the
// signing door and the move-out door assemble a statement the SAME way.
...
export async function previewChargeRun(
  ...
  // AND NOT A MONTH THAT HAS NOT STARTED. The rent screen hides the forward
  // link past the current month; a typed `?month=` reached this door with
  // no rule behind it and raised February on 28 January.
  const tooSoon = notYetBillableRefusal(month, todayLakeDate(), prettyMonth);
  if (tooSoon) return { ok: false, error: tooSoon };
  ...
  if (plan.toBill.length === 0) return { ok: true, plan };

  // WHAT COMES OFF FROM MONEY ON ACCOUNT (0167) — planned by the same
  // function the run calls, over the same rows, so "$1,085.06 of it already
  // on account" is the figure the run then applies. Read after the plan, for
  // the households actually being billed.
  //
  // OLDEST BILL FIRST (R1). The household's OLDER open bills are in the plan
  // too, ahead of the one being raised: a cheque on account settles January's
  // arrears before it touches February, so the figure promised here for
  // February is what is left after January — the same order the run writes.
  const renterOf = new Map((stays ?? []).map((s) => [s.id as string, (s.renter_id as string) ?? null]));
  const billed = plan.toBill.map((b) => renterOf.get(b.reservationId) ?? "").filter(Boolean);
  const srcRes = await onAccountSources(admin, parkId, billed);
  if (srcRes.error) {
    return { ok: false, error: readFailedMessage("the money households have on account", srcRes.error, { money: true }) };
  }
  if (srcRes.sources.length === 0) return { ok: true, plan: withOnAccount(plan, new Map()) };
  const olderRes = await openBillsFor(admin, parkId, billed);
  if (olderRes.error) {
    return { ok: false, error: readFailedMessage("the bills still open", olderRes.error, { money: true }) };
  }
  const allocations = planSettlement(
    [
      ...olderRes.bills,
      ...plan.toBill.map((b) => ({
        key: b.reservationId, renterId: renterOf.get(b.reservationId) ?? null, owing: b.amount,
        periodMonth: month, dueOn: `${month}-99`,
      })),
    ],
    srcRes.sources,
  );
  // THE SAME PARTITION THE RUN MAKES (splitApplied): a planned dollar keyed
  // on a bill being raised is that bill's; one keyed on an older open bill
  // is named with its month. The older ones used to be planned here and
  // discarded, so the sentence he approved from said "$57.47 of it already
  // on account" while the run moved $600 — January first, then February.
  const olderMonth = new Map(olderRes.bills.map((b) => [b.key, b.periodMonth]));
  const parted = splitApplied(
    plannedByKey(allocations).byKey,
    new Set(plan.toBill.map((b) => b.reservationId)),
    (key) => olderMonth.get(key),
  );
  return { ok: true, plan: withOnAccount(plan, plannedByKey(allocations).byKey, parted.toOlderBills) };
}
...
export async function runCharges(
  ...
  const tooEarly = preCutoverRefusal(
    month, (park?.cutover_date as string | null) ?? null, prettyMonth);
  if (tooEarly) return { ok: false, error: tooEarly };
  // The same rule as the preview, checked again here rather than trusted:
  // a month that has not started is not raised early by a typed URL.
  const tooSoon = notYetBillableRefusal(month, todayLakeDate(), prettyMonth);
  if (tooSoon) return { ok: false, error: tooSoon };
  ...
  // MONEY ON ACCOUNT COMES OFF THE BILLS JUST RAISED (0167). The owner's
  // words: "need it applied to the months if there is a prepay." For each
  // billed household with money on account, settle its OPEN bills oldest
  // first — an older month still owing before the one raised today (R1) —
  // through the one door every payment door uses, which plans with the same
  // function the preview did, and writes one row per (payment, bill) so a
  // refusal on one bill leaves the others applied.
  //
  // THE BILLS STAND WHATEVER HAPPENS HERE. A failed read or a refused row is
  // named in the sentence; the money stays on account and the office can
  // apply it by hand. What is never allowed is a quiet outcome — a bill
  // raised, a cheque in the drawer, and Lot 7 chased on the 4th.
  const kept = (raised ?? []).filter((r) => !rolledBack.has(r.id as string));
  const keptIds = new Set(kept.map((c) => c.id as string));
  const lotOfCharge = new Map(kept.map((c) => [
    c.id as string,
    (lotById.get(stayById.get(c.reservation_id as string)?.park_lot_id as string)?.lot_number as string) ?? "?",
  ]));
  const billedHouseholds = kept
    .map((c) => (stayById.get(c.reservation_id as string)?.renter_id as string | null) ?? "")
    .filter(Boolean);
  let fromOnAccount = 0;
  let toOlderBills: { periodMonth: string; amount: number }[] = [];
  const applyProblems: string[] = [];
  const settled = await settleOnAccount(admin, parkId, billedHouseholds, "run", null);
  if ("error" in settled) {
    console.error(`[runCharges] couldn't read ${settled.what}:`, settled.error);
    applyProblems.push(
      `We couldn't read ${settled.what}, so no money on account was put against these bills — ` +
      "the bills stand, the money stays on account, and you can apply it from \"Money not against a bill\".",
    );
  } else {
    const monthOf = new Map(settled.bills.map((b) => [b.key, b.periodMonth]));
    // THE SAME PARTITION THE PREVIEW MADE (splitApplied): a dollar on a bill
    // raised this morning, or on an older open bill — named with its month.
    const parted = splitApplied(settled.applied, keptIds, (key) => monthOf.get(key));
    fromOnAccount = parted.fromOnAccount;
    toOlderBills = parted.toOlderBills;
    for (const f of settled.failed) {
      const lot = lotOfCharge.get(f.key);
      applyProblems.push(
        `${money(f.amount)} of ${lot ? `Lot ${lot}'s` : "a household's"} money on account couldn't be put against ` +
        `${keptIds.has(f.key) ? "its bill" : `its ${prettyMonth(monthOf.get(f.key) ?? "")} bill`} — ` +
        "the bill stands and the money stays on account; apply it from \"Money not against a bill\".",
      );
    }
  }
  ...
  return {
    ...
    total,
    signal:
      `${keptRows.length} ${keptRows.length === 1 ? "bill" : "bills"} raised for ${prettyMonth(month)} — ${money(total)}` +
      // WHERE MONEY ON ACCOUNT WENT — older open months first (R1), then the
      // bills just raised — in the ONE clause the preview used, so the
      // figure he approved from is the figure he reads now: "$1,085.06 of
      ...
      (applyProblems.length > 0 ? ` ⚠️ ${applyProblems.join(" ")}` : ""),
  };
}
...
export async function recordPayment(
  ...
  if (onAccountCents > 0 && !renterId) {
    ...
    return {
      ok: false,
      error:
        `That bill only has ${money(against)} left on it and isn't tied to a household, ` +
        `so the other ${money(onAccount)} can't be held on account here. Record ` +
        `${money(against)} against it, and put the rest under "Money not against a bill" ` +
  ...
  if (onAccountCents > 0) {
    ...
    // household, kind 'rent'. getHeldMoney lists it, and the run or
    // applyOnAccount puts it against a bill as an allocation (0167) — the row
    // itself never moves. Its own key, derived from the form's, so a double
    // tap collides on both rows and neither lands twice.
    rows.push({
      charge_id: null,
      park_id: parkId,
      renter_id: renterId,
      ...
      // shape this is.
      note: againstCents > 0
        ? `Paid over the ${prettyMonth(String(charge.period_month ?? ""))} bill — the bill took ${money(against)} of ${money(amount)}.`
        : `The ${prettyMonth(String(charge.period_month ?? ""))} bill was already settled, so all ${money(amount)} is on account.`,
      ...
      idempotency_key: key ? onAccountKey(key) : null,
    });
  }
  ...
  const { data: written, error } = await admin.from("park_payments").insert(rows)
    .select("id, receipt_no, fee_amount, charge_id");
  if (error) {
  ...
  if (settlesClaimId && againstCents === 0) {
    const { error: claimErr } = await admin
      ...
      .update({
        ...
        resolution: "matched",
        resolution_note: `The office confirmed collecting ${money(onAccount)}. That bill was already settled, so it was recorded on account.`,
  ...
  // MONEY ON ACCOUNT SETTLES THE HOUSEHOLD'S OLDEST OPEN BILLS, NOW (0167,
  // R1). Two shapes land here: the household pays again while money of
  // theirs is already in the office (a cheque taken before the bill existed,
  // the excess over last month) — it goes against THIS bill if it still owes;
  // and the excess of a split just recorded on account goes against any
  // OLDER bill still open, oldest first. One door for both (settleOnAccount),
  // so the ledger never reads "$57.47 outstanding" on a household who has
  // overpaid. The payment is already recorded, so a failure here cannot
  // refuse; it is named in the sentence and the office applies it by hand.
  //
  // THE EXCESS NEVER PAYS THE BILL IT CAME IN OVER: that bill is settled by
  // the direct part (the excess exists only when the payment covered it), so
  // it owes nothing and the plan skips it. A future bill does not exist yet;
  // the run applies what is left the morning it is raised.
  let fromOnAccount = 0;
  /** Where the excess just recorded went — its own lines only. */
  let elsewhere = "";
  /** The same, as lines for the receipt. */
  let excessLines: AllocationLine[] = [];
  ...
  if (renterId) {
    ...
    if ("error" in settled) {
      console.error(`[recordPayment] couldn't read ${settled.what}:`, settled.error);
      applyProblem = `We couldn't read ${settled.what}, so none of their money on account was put against a bill.`;
    } else {
      fromOnAccount = settled.applied.get(chargeId) ?? 0;
      // Two sentences, two filters: the excess's lines are the excess's; an
      // older cheque's lines are the older cheque's. Neither counts the other.
      const excessId = acctRow ? (acctRow.id as string) : null;
      elsewhere = excessId ? describeSettlement(settled, (l) => l.paymentId === excessId) : "";
      const monthOf = new Map(settled.bills.map((b) => [b.key, b.periodMonth]));
      excessLines = settled.lines
  ...
  // WHAT THE OFFICE ALREADY HELD THAT IS ON THIS BILL — for the paper, ALL
  // of it. `fromOnAccount` above is what settleOnAccount applied INSIDE this
  // call; a bill part-paid from money on account on an EARLIER day ($200 of
  // a $542.53 January, applied on the 3rd; $342.53 cash keyed on the 6th)
  // was already in `charge.paid_total`, so nothing was applied today and the
  // receipt read "Amount $342.53 / Against January rent — $542.53 / nothing
  // further owing" with no line explaining the difference, on the only copy
  // the household keeps. Read AFTER the settlement so today's lines are in
  // it too: the live allocations on this bill from payments that still
  // stand, the filter park_charge_paid_total applies. Its own variable —
  // `fromOnAccount` still feeds the balance fallback below (where the
  // pre-insert paid_total already holds the earlier lines) and the toast's
  // "went against it too", which is about THIS act. The money is recorded,
  // so a failed read degrades to the same-call figure and is logged.
  let onBillFromAccount = fromOnAccount;
  const onBillRes = await admin
    .from("park_payment_allocations")
    .select("amount, park_payments!inner(reversed_at, returned_at)")
    .eq("charge_id", chargeId)
    .is("removed_at", null);
  if (onBillRes.error) {
    console.error("[read failed] what was already on this bill from money on account:", onBillRes.error);
  } else {
    onBillFromAccount = (onBillRes.data ?? [])
      .filter((a) => {
        const p = Array.isArray(a.park_payments) ? a.park_payments[0] : a.park_payments;
        const pay = p as { reversed_at?: unknown; returned_at?: unknown } | null;
        return pay != null && pay.reversed_at == null && pay.returned_at == null;
      })
      .reduce((t, a) => t + cents(Number(a.amount ?? 0)), 0) / 100;
  }

  // WHAT OF THE EXCESS IS STILL HELD — the database's answer, read after the
  // settlement, never `onAccount − applied` here (that would count money the
  // household had on account before today as today's). Only read when some
  // of it went somewhere; untouched, all of it is held. A FAILED READ IS
  // NULL, not the whole excess: "$57.47 stays on account" about money $40 of
  // which just went on December is the sentence this used to print, and the
  // receipt printed the same figure as held. Null prints "what's left".
  let stillHeld: number | null = onAccount;
  if (onAccountCents > 0 && excessLines.length > 0 && acctRow) {
    const heldRes = await admin
      .from("park_on_account_payments").select("remaining").eq("payment_id", acctRow.id as string).maybeSingle();
    if (heldRes.error) {
      console.error("[read failed] what is still on account:", heldRes.error);
      stillHeld = null;
    } else {
      stillHeld = Number(heldRes.data?.remaining ?? 0);
    }
  }
  ...
  const balance = after
    ...
    : Number(charge.amount) - Number(charge.paid_total) - against - fromOnAccount;
  ...
  const receipt: ReceiptLines = {
    ...
    // AND WHERE THE EXCESS WENT, when it went somewhere the moment it was
    // recorded (an older bill still open): the paper says "$40.00 to
    // December 2026, $17.47 on account", not "held by the office" about
    // money that is already on December.
    onAccount: onAccountCents > 0
      ? {
          amount: onAccount,
          receiptNo: (acctRow?.receipt_no as number) ?? null,
          // `remaining` only when it was read; receiptBody prints "wasn't
          // read when this was printed" for applied lines with no figure.
    ...
    // beside what they handed over today OR on an earlier day. On the paper,
    // or "nothing further owing" on a $542.53 bill for a $342.53 payment is
    // a figure the household cannot reconcile to what they paid.
    fromOnAccount: onBillFromAccount > 0 ? onBillFromAccount : null,
  ...
  return {
    ...
    signal: onAccountCents > 0
      // BOTH LINES, and where the second one is. Money on account settles
      // the household's oldest open bill the moment it is recorded (R1) and
      // comes off the next bill the run raises; what it did just now is
      // said, or the office reads "$57.47 on account" about money that is
      // already on December. What happens to what is still held is the one
      // promise (onAccountPromise), keyed on the tenancy: the month after
      // this bill when a next bill is coming, theirs to have back when
      // nothing more bills, and no promise at all when that could not be
      // read — the clause sits right after "on account", so the unknown
      // shape is a sentence that simply stops there.
      ? (againstCents > 0
          ? `${money(amount)} received — ${money(against)} against ${monthLabel}, ` +
            `${money(onAccount)} on account`
          : `${money(amount)} received — ${monthLabel} was already settled, so all of it is on account`) +
        (elsewhere
          ? `. Of that, ${elsewhere} — ${
              stillHeld == null
        ...
        (claimStillOpen
          ...
          : "") +
        (applyProblem ? ` ⚠️ ${applyProblem}` : "")
      : (fromOnAccount > 0
          // WHAT THE OFFICE ALREADY HELD WENT IN TOO. Said, or "settled" reads
          // as a $200 cash payment settling a $542.53 bill.
          ? `Recorded, and ${money(fromOnAccount)} they had on account went against it too` +
            (balance > 0 ? ` — ${money(balance)} still outstanding.` : " — that one's settled.")
          : balance > 0
            ? `Recorded. ${money(balance)} still outstanding.`
            : "Recorded — that one's settled.") +
        (older ? ` And ${older} from money they had on account.` : "") +
        (applyProblem ? ` ⚠️ ${applyProblem}` : ""),
  };
}
...
export async function voidCharge(
  ...
  // WHAT IS ON THE BILL DECIDES WHAT HAPPENS TO IT.
  //
  // "Cancel this bill" is a row control on every open ledger line of the rent
  // screen, paid or not, so this is a mis-tap away. Money on account put against it (0167) REFUSES the void:
  // that money is another payment's, it comes off the bill with a reason
  // first (R3), and the database refuses the void by name while a line
  // stands (0169). Money TAKEN against it does not refuse: cancelling the
  // bill releases it onto the household's account — derived, the payment
  // row never moves (0169) — and the sentence says where it went and the
  ...
  // database still refuses that void — but he would get "Couldn't cancel
  // that" forever instead of the sentence naming the door.
  if (existingRes.error) {
  ...
  if (existing && Number(existing.paid_total) > 0) {
    ...
    // against this bill AND money on account the run or the office put
    // against it (0167). The second kind was not "taken against this bill"
    // by anybody at a window, and the door for it is taking the allocation
    // back off the bill — the sentence has to name it. A failed read here
    // refuses: it must not instruct the wrong door, or void a bill with a
    // line it could not see.
    //
    // BOTH READ THROUGH THE PAYMENT'S STANDING — the filter park_charge_
    // paid_total applies. An allocation SURVIVES a reversal as record, so a
    // $442.53 line from a cheque that has since bounced is not money on this
    // bill: reading it would name "Take it off this bill" on a line no
    // screen lists (the reversed payment is not in the view).
    const [allocRes, directRes] = await Promise.all([
      admin
        .from("park_payment_allocations")
        .select("amount, park_payments!inner(reversed_at, returned_at)")
        .eq("charge_id", chargeId)
    ...
    if (allocRes.error) {
      return { ok: false, error: readFailedMessage("what's been put against that bill", allocRes.error, { money: true }) };
    }
    if (directRes.error) {
      return { ok: false, error: readFailedMessage("what's been paid against that bill", directRes.error, { money: true }) };
    }
    const stands = (a: { park_payments?: unknown }) => {
      // PostgREST embeds a many-to-one as an object; guard the array shape.
    ...
    const allocated = allocatedTotal(
      (allocRes.data ?? []).filter(stands).map((a) => ({ periodMonth: "", amount: Number(a.amount ?? 0) })),
    );
    directRows = ((directRes.data ?? []) as Array<{ id: unknown; method: unknown }>)
      .map((p) => ({ id: p.id as string, method: (p.method as string | null) ?? null }));
    const month = prettyMonth(String(existing.period_month ?? ""));
    if (allocated > 0) {
      return {
        ok: false,
        error:
          `${money(allocated)} of this bill was settled from money on account — take that off it first ` +
          `(under "Money not against a bill", "Take it off this bill" on the ${month} line).`,
      };
    }
    // paid_total says money is on it and neither read found any standing —
    // the bill's figure is stale or the rows are somewhere this cannot see.
    // Say what the ledger says rather than releasing money it cannot name.
    if (directRows.length === 0) {
      return {
        ok: false,
        error: `${money(Number(existing.paid_total))} is recorded against this bill, so it can't be cancelled — check its payments first.`,
      };
    }
  }
  ...
  // WHERE THE MONEY TAKEN AGAINST IT WENT (0169). The figure is the view's
  // `remaining` for the rows this bill just released — one definition of
  // "still on account", net of refunds — read after the void, never amount
  // minus something here. No auto-settle: the same posture as taking a
  // line off a bill (unapplyAllocation) — the office decides what the
  // money does next, from "Money not against a bill". A failed read is said
  // as one: the money IS on account, and silence would be the lie.
  let onAccount = "";
  if (directRows.length > 0) {
    const [viewRes, whoRes] = await Promise.all([
      admin
        .from("park_on_account_payments")
        .select("payment_id, remaining, amount, refunded")
        .eq("released_from_charge_id", chargeId),
      existing?.renter_id
        ? admin.from("park_renters").select("display_name").eq("id", existing.renter_id as string).maybeSingle()
        : Promise.resolve({ data: null, error: null }),
    ]);
    if (whoRes.error) console.error("[read failed] the household on that bill:", whoRes.error);
    const who = String(whoRes.data?.display_name ?? "").trim() || "that household";
    // The doors out, by the money's rail: hand-keyed money goes back across
    // the window and is recorded from its line (0168); card and ACH money
    // goes back through the processor (0142).
    const byHand = directRows.some((p) => p.method !== "card" && p.method !== "ach");
    const byRail = directRows.some((p) => p.method === "card" || p.method === "ach");
    const doors = [
      ...(byHand ? [`hand it back from "Money not against a bill" on the Rent screen`] : []),
      ...(byRail ? [`send it back through "Refund to card" on Statements`] : []),
    ].join(", or ");
    if (viewRes.error) {
      console.error("[read failed] what the cancelled bill released:", viewRes.error);
      onAccount = ` What was taken against it is on account for ${who} now, though the figure couldn't be read — check "Money not against a bill" on the Rent screen.`;
    } else {
      const sum = (col: "remaining" | "amount" | "refunded") =>
        (viewRes.data ?? []).reduce((t, r) => t + Math.round(Number(r[col] ?? 0) * 100), 0) / 100;
      const released = sum("remaining");
      // THREE FIGURES, ALL THE VIEW'S. "The $300.00 taken against it" named
      // the remainder as the amount taken when $400 was taken and $100 had
      // already gone back to the card. When anything was refunded, the
      // whole truth is said from the view's own columns — never amount
      // minus refunds here; when nothing was, what was taken IS what is on
      // account and the plain sentence stays.
      const refunded = sum("refunded");
      const figure = refunded > 0
        ? `The ${money(sum("amount"))} taken against it, less the ${money(refunded)} already refunded, leaves ${money(released)} on account for ${who} now`
        : `The ${money(released)} taken against it is on account for ${who} now`;
      // WHAT HAPPENS TO IT NEXT is a promise, and it is decided on the bill
      // just cancelled, not assumed. "It comes off their next bill" was
      // said to a household that had moved out with their final month
      // billed — a bill the park will never raise — while the held panel
      // this sentence points at says "nothing more bills for them". Three
      // honest shapes: the run would bill this month again (a cancelled
      // in-stay month of a household still here, or one who has gone — the
      // run bills a void month again on an ended row that carries its last
      // day, and takes the money back off account when it does); nothing
      // more bills (theirs to have back); or the next bill. A failed read
      // of either fact makes no promise at all — the money IS on account,
      // and "comes off the next bill" is the one thing this must not guess.
      const next = await whatBillsNext(admin, parkId, existing, chargeId);
      const promise = next === "unknown"
        ? ""
        : next === "billsAgain"
          ? `it comes off ${prettyMonth(String(existing?.period_month ?? ""))} when you bill it again, or `
          : next === "nothingMore"
            ? "nothing more bills for them; "
            : "it comes off their next bill, or ";
      onAccount = ` ${figure} — ${promise}${doors}.`;
    }
  }
  ...
  return {
    ...
    signal:
      `Cancelled.${onAccount}` +
      (n > 0 ? ` ${n} cost ${n === 1 ? "share is" : "shares are"} waiting again for the next bill.` : ""),
  };
}

/**
 * WHAT WILL BILL NEXT for the household whose bill voidCharge just cancelled
 * — the three-way fact its promise keys on. "billsAgain" when the run would
 * raise the cancelled month again on that link (classifyForRun, the run's
 * own rule, with the void month billable again and no other live bill for
 * it); "nothingMore" when the tenancy has ended and their last month is
 * billed (lib/tenancy-facts — the held panel's and the resident receipt's
 * rule); "nextBill" otherwise; "unknown" on any failed read, which the
 * caller turns into no promise at all.
 */
async function whatBillsNext(
  admin: ReturnType<typeof createServiceClient>,
  parkId: string,
  bill: { reservation_id?: unknown; renter_id?: unknown; period_month?: unknown; amount?: unknown } | null,
  voidedId: string,
): Promise<"billsAgain" | "nothingMore" | "nextBill" | "unknown"> {
  const reservationId = (bill?.reservation_id as string | null) ?? null;
  const month = String(bill?.period_month ?? "");
  if (!reservationId || !month) return "unknown";
  const [linkRes, liveRes] = await Promise.all([
    admin.from("lot_reservations").select("status, during, term, moved_out_on").eq("id", reservationId).maybeSingle(),
    admin.from("park_charges").select("id")
      .eq("park_id", parkId).eq("reservation_id", reservationId).eq("period_month", month)
      .neq("status", "void").neq("id", voidedId),
  ]);
  if (linkRes.error || liveRes.error || !linkRes.data) {
    console.error("[read failed] whether the cancelled month bills again:", linkRes.error ?? liveRes.error ?? "no such tenancy");
    return "unknown";
  }
  const link = linkRes.data;
  // The run's own filter before its classification: an ended row bills
  // again only when it carries the day they left (runCharges, planRun).
  const runReads = link.status !== "ended" || link.moved_out_on != null;
  const why = !runReads ? "movedOut" : classifyForRun(
    {
      reservationId,
      lotNumber: "",
      // The bill's own amount stood for the statement total when it was
      // raised; classifyForRun asks only whether a rent is set.
      amount: bill?.amount != null ? Number(bill.amount) : null,
      range: parseDaterange(link.during as string),
      term: (link.term as string | null) ?? null,
      status: (link.status as string | null) ?? null,
    },
    month,
    new Set((liveRes.data ?? []).length > 0 ? [reservationId] : []),
  );
  if (why === "bill") return "billsAgain";
  const renterId = (bill?.renter_id as string | null) ?? null;
  if (!renterId) return "unknown";
  try {
    const facts = await tenancyFactsFor(admin, [renterId]);
    return nothingMoreBills(facts.get(renterId)) ? "nothingMore" : "nextBill";
  } catch (e) {
    console.error("[read failed] whether anything more bills for them:", e);
    return "unknown";
  }
...
export async function getLedger(parkId: string, month?: string): Promise<LedgerPage | null> {
  ...
  // WHAT THE RECORD-PAYMENT FORM ON EACH ROW HAS TO KNOW ABOUT THE
  // HOUSEHOLD — the ⊕ window's three facts (paymentTargets, pos-actions),
  // from the SAME readers, so the two forms cannot describe one household
  // in one state two ways. The form here read the bill alone and said
  // nothing about held money — "Record it" under a box pre-filled to
  // $542.53, for a household whose own $542.53 was in the drawer — while
  // the window said that money would cover it and named the door.
  //   Money on account: onAccountSources — the view's `remaining`, the
  //   settlement door's own read. Never heldOnAccountFor: its figure carries
  //   deposits, and a deposit is not money that pays a rent bill.
  //   Open bills: openBillsFor, across EVERY month — this loader is scoped
  //   to one, and an older bill still open is exactly what decides where an
  //   over-payment goes the moment it is recorded (R1).
  // Both throw the way every read above does: "nothing on account" for
  // everybody is how the office takes the same rent twice, and a bare `[]`
  // here would be that.
  const heldRes = await onAccountSources(admin, parkId, renterIds);
  const heldSources = mustRead(
    "the money households have on account",
    { data: heldRes.sources, error: heldRes.error as { message?: string; code?: string } | null },
  );
  const heldCentsOf = new Map<string, number>();
  for (const src of heldSources) {
    if (!src.renterId) continue;
    heldCentsOf.set(src.renterId, (heldCentsOf.get(src.renterId) ?? 0) + Math.round(src.remaining * 100));
  }
  const billsRes = await openBillsFor(admin, parkId, renterIds);
  const openBills = mustRead(
    "the bills still open",
  ...
  //   then makes NO promise for anybody (the window does the same), never a
  //   guessed one.
  ...
  // By charge id: a Charge names its household and does not carry its id.
  ...
  for (const c of data ?? []) {
    ...
    householdMoney.set(c.id as string, {
      ...
      nothingMoreBills: facts && rid ? nothingMoreBills(facts.get(rid)) : null,
    });
  }
 ...
 * and 0072 refused to void a charge that has money against it. Type $4,395
 ...
 *
 * ONE CHEQUE, BOTH ROWS. A payment for more than the bill is two rows
 * (recordPayment: the bill's share, the rest on account under the same key
 * + ":onaccount"). Whichever half the office taps — the bill row from
 * Statements, the on-account row from the held-money panel — the other half
 * is taken back in the same update with the same reason and timestamp, and
 * the sentence names every bill that reopened, including the months the
 * on-account half had been put against.
 *
 * A ROW AGAINST A CANCELLED BILL (0169) is money on account: reversing it
 * reopens nothing on that bill — it is void — but every month its released
 * money had been put against (the part month) goes back to owing, and the
 * sentence names them and says the cancelled bill reopens nothing.
 */
export async function reversePayment(
  ...
  // MONEY ALREADY HANDED BACK CANNOT BE UNSAID. Reversing means "this never
  // happened", and the money demonstrably did go back out across the window
  // — a deposit returned (0102), rent on account handed back (0168). A
  // returned_amount on a reversed row would be the ledger holding two
  // contradictory facts about the same cash; the database refuses it too.
  // Read off the row itself, in the same select, so a failed read refuses
  // above rather than falling through here.
  //
  // AND NO DOOR OUT IS NAMED, BECAUSE THERE IS NONE YET. A cheque that
  // bounces AFTER part of it was handed back has no exit: a hand-back is
  // recorded once and nothing un-records it (0168 says so in its own
  // header). The sentence says that plainly — the office must not go
  // looking for a button, and "try again" would never come true. Whether
  // a hand-back gets its own correction-with-a-reason is the owner's call.
  if (pay.returned_on) {
    return {
      ok: false,
      error: pay.kind === "deposit"
        ? "That deposit was already returned — reversing it would contradict the record."
        : `The record shows ${money(Number(pay.returned_amount ?? 0))} of that handed back to them on ${dayInWords(String(pay.returned_on))} — reversing it would contradict the record, and there's no way yet to undo a hand-back.`,
    };
  }

  // THE OTHER HALF OF A SPLIT CHEQUE. recordPayment writes $600 on a
  // $542.53 bill as TWO rows in one insert — the bill's share under the
  // form's key, the $57.47 on account under key + ":onaccount". They are one
  // cheque: a bounce takes both back, a transposed digit was transposed on
  // both. This used to reverse ONE row by id and know nothing of the other,
  // so after a bounced cheque the $57.47 stayed standing under "Money not
  // against a bill" and the next run put money that never arrived against
  // February — automatically. The sibling is found through the ONE spelling
  // of the key (splitSiblingKey), park-scoped, still standing; a failed read
  // refuses, because reversing half of one cheque and saying nothing about
  // the other is the exact lie this exists to prevent.
  let sibling: { id: string; amount: number; charge_id: string | null } | null = null;
  const sibKey = pay.kind === "deposit" || pay.kind === "amenity"
    ? null
    : splitSiblingKey((pay.idempotency_key as string | null) ?? null, pay.charge_id);
  if (sibKey) {
    const sibRes = await admin
      .from("park_payments")
      .select("id, amount, charge_id, returned_on, returned_amount")
      .eq("idempotency_key", sibKey)
      .eq("park_id", parkId)
      .is("reversed_at", null)
      .maybeSingle();
    if (sibRes.error) {
      return { ok: false, error: readFailedMessage("the other half of that payment", sibRes.error, { money: true }) };
    }
    if (sibRes.data) {
      // The other half handed back (0168) takes the whole payment with it:
      // one statement reverses both rows, the database refuses the
      // handed-back one, and "try again" would never become true. "The
      // payment", never "the cheque" — recordPayment writes the same two
      // rows for a $600 cash split, and a method word hardcoded here was
      // the lie J's fix removed from the confirm one screen over.
      if (sibRes.data.returned_on) {
        return {
          ok: false,
          error: `The record shows ${money(Number(sibRes.data.returned_amount ?? 0))} of the on-account half of that payment handed back to them on ${dayInWords(String(sibRes.data.returned_on))} — reversing the payment would contradict the record, and there's no way yet to undo a hand-back.`,
        };
      }
      sibling = {
        id: sibRes.data.id as string,
        amount: Number(sibRes.data.amount ?? 0),
        charge_id: (sibRes.data.charge_id as string | null) ?? null,
      };
    }
  }

  // WHETHER EACH ROW IS ON ACCOUNT — the view's answer (0167, 0169), never
  // `charge_id` alone. A row against a CANCELLED bill is money on account:
  // its charge_id still names the void bill, so `charge_id` reads it as
  // "against a bill" and the months its released money settled (the part
  // month) would go unnamed while they silently reopen — and "the January
  // bill is outstanding again" would be said of a bill nobody owes. Both
  // halves of a split are asked in one read, because either half can be
  // the released one depending on which the office tapped.
  const viewRes = await admin
    .from("park_on_account_payments")
    .select("payment_id, remaining, released_from_charge_id")
    .in("payment_id", [paymentId, ...(sibling ? [sibling.id] : [])]);
  if (viewRes.error) {
    return { ok: false, error: readFailedMessage("whether that payment is on account", viewRes.error, { money: true }) };
  }
  const releasedRow = (id: string) => (viewRes.data ?? []).some((v) => v.payment_id === id && v.released_from_charge_id != null);
  const released = releasedRow(paymentId);
  const sibReleased = sibling != null && releasedRow(sibling.id);

  // WHICH ROWS' LINES REOPEN: the tapped row's when it is money on account —
  // no bill, or a bill since cancelled (released) — and the sibling's when
  // IT is. A released $542.53 with a $57.47 sibling is reversed in ONE
  // statement below; reading only the tapped row's lines would leave the
  // other's month to reopen unnamed, the exact lie 0167 § 20 warned about.
  const ownAcct = !pay.charge_id || released;
  const sibAcct = sibling != null && (!sibling.charge_id || sibReleased);
  const acctRowIds: string[] = [
    ...(ownAcct && (pay.kind ?? "rent") === "rent" ? [paymentId] : []),
    ...(sibAcct ? [sibling!.id] : []),
  ];
  const billChargeId = (pay.charge_id as string | null) ?? sibling?.charge_id ?? null;

  // WHERE THE MONEY HAD GONE (0167). A payment on account may have been put
  // against several bills; reversing it reopens every one of them — the
  // trigger does that — and the sentence has to name them, or the office
  // reads "off the household's account" while three months just went back
  // to owing. For a split, the on-account half's lines are read whichever
  // half was tapped. Read BEFORE the write, and refuse on a failed read:
  // nothing has moved yet, and a sentence that names no months about a
  // reversal that reopened three is the lie this exists to prevent.
  let hadGone: Array<ReopenedLine & { paymentId: string }> = [];
  let allocs: Array<{ payment_id: unknown; charge_id: unknown; amount: unknown }> = [];
  if (acctRowIds.length > 0) {
    // LIVE LINES ONLY. An allocation the office already took back off its
    // bill (removed_at set) is not reopening anything — naming that month
    // would send somebody looking for a balance that did not move.
    const allocRes = await admin
      .from("park_payment_allocations")
      .select("payment_id, charge_id, amount")
      .in("payment_id", acctRowIds)
      .is("removed_at", null);
    if (allocRes.error) {
      return { ok: false, error: readFailedMessage("which bills that payment had been put against", allocRes.error, { money: true }) };
    }
    allocs = allocRes.data ?? [];
  }
  // The months, for the sentence: every bill an allocation reopens, and the
  // bill the direct half was against. One read for all of them — with each
  // bill's own amount and frozen lines, because the part month a released
  // row settled shares the cancelled bill's month (0169) and the sentence
  // has to name the two apart: the cancelled bill by its amount, the
  // re-raise by its amount and basis. Never pay.amount for either — a split
  // or partial cheque differs from both bills.
  const monthIds = [...new Set([...allocs.map((a) => a.charge_id as string), ...(billChargeId ? [billChargeId] : [])])];
  ...
  if (monthIds.length > 0) {
    const chRes = await admin
      ...
      .in("id", monthIds);
    if (chRes.error) {
      return { ok: false, error: readFailedMessage("the bills that payment had been put against", chRes.error, { money: true }) };
    }
    const billOf = new Map((chRes.data ?? []).map((c) => [c.id as string, c]));
    billMonth = billChargeId ? ((billOf.get(billChargeId)?.period_month as string | undefined) ?? null) : null;
    billAmount = billChargeId && billOf.get(billChargeId)?.amount != null ? Number(billOf.get(billChargeId)!.amount) : null;
    // A line in the bill's own month is the bill raised again for it ONLY
    // when that bill is cancelled: on a live January the on-account half
    // of a short cheque can be put against the same live bill, and that
    // line is January itself, not a re-raise.
    const releasedMonth = (pay.charge_id ? released : sibReleased) ? billMonth : null;
    hadGone = allocs.map((a) => {
      const bill = billOf.get(a.charge_id as string);
      // Marked as the re-raise only when this line's month is the cancelled
      // bill's — the one decision of which line collides, made in
      // lib/allocations.
      ...
      return {
        ...
        ...line,
        billAmount: bill?.amount != null ? Number(bill.amount) : null,
      };
    });
  }

  // ONE UPDATE OVER BOTH IDS — same reason, same timestamp. It is one cheque,
  // and the record must not be able to say half of it bounced.
  const ids = [paymentId, ...(sibling ? [sibling.id] : [])];
  const { data: done, error } = await admin
    ...
    .select("id");
  if (error) return { ok: false, error: "Couldn't record that — try again." };
  // WHAT THE WRITE ACTUALLY REACHED. Between the read and the write somebody
  // else may have taken a half back: the row itself (then nothing here was
  // reversed by this act — say so, never "taken back" about nothing) or the
  // sibling (then this act reversed one row, and the sentence must not name
  // the other half or the months its earlier reversal already reopened).
  const reached = new Set((done ?? []).map((d) => d.id as string));
  if (done && !reached.has(paymentId)) {
    return { ok: false, error: "That one was just taken back by somebody else." };
  }
  const withSibling = sibling != null && (done == null || reached.has(sibling.id));
  if (sibling && !withSibling) {
    console.error("[reversePayment] the other half of the cheque had already been taken back:", sibling.id);
  }
  ...
  const whole = withSibling ? Math.round((amount + sibling!.amount) * 100) / 100 : amount;
  // A sibling this act did not reach reopened nothing: its months, or the
  // bill it was against, went back to owing when IT was taken back. The
  // tapped row's own lines are always its own act — a released row's
  // included, or the part month it settled reopens unnamed — and the
  // sibling's only when the write reached it.
  ...
  return {
    ...
    signal: reversalSentence({
      ...
      split: withSibling
        ? pay.charge_id
          ? { tapped: "bill", against: amount, onAccount: sibling!.amount }
          : { tapped: "on_account", against: sibling!.amount, onAccount: amount }
        : null,
      hadGone: hadGone.filter((l) => reopenedBy.has(l.paymentId)).map(({ paymentId: _p, ...line }) => line),
      // The bill the sentence names is the split's bill half — the tapped
      // row's, or the sibling's — and it reopens nothing when it is void.
      billCancelled: pay.charge_id ? released : withSibling && sibReleased,
      billAmount,
    }),
  };
}
...
export interface RefundableState {
  ...
  refusal: string | null;
  /** The rail the money took — 'card' or 'ach' — so the sentence names it. */
  method: string | null;
  /**
   * The bill it was against is cancelled (0169): the money is on account,
   * and a refund of it reopens nothing. Read off the view, never charge_id.
   */
  released: boolean;
}
...
export async function refundableOn(parkId: string, paymentId: string): Promise<RefundableState | { error: string }> {
  ...
  const payRes = await admin
    ...
    .select("id, amount, fee_amount, method, reference, reversed_at, returned_at, charge_id, kind")
    .eq("id", paymentId)
    .eq("park_id", parkId)
    .maybeSingle();
  ...
  // MONEY ALREADY PUT AGAINST BILLS IS NOT REFUNDABLE (0167). A payment on
  // account that settled January and February has only its unapplied rest to
  // send back; the guard refuses the rest by name, and this is the sentence.
  //
  // "ON ACCOUNT" MEANS "IN THE VIEW" — never `charge_id` alone. A row against
  // a CANCELLED bill still carries the bill's id and IS on account (0169,
  // released): its ceiling is what is left after the part month it settled,
  // and a refund of it reopens nothing.
  //
  // THE CEILING IS THE VIEW'S `remaining`, not a subtraction here — the one
  // definition of "what is left on a payment" lives in park_payment_remaining
  // (amount − live allocations − refunds − handed back), and a second copy
  // in JavaScript is the shape this codebase keeps finding as a bug. A
  // payment against a LIVE bill has no row in the view: its ceiling is
  // amount − refunds, as before. A failed read is not "nothing applied" —
  // that would offer the whole payment back over money that is on three bills.
  const viewRes = await admin
    .from("park_on_account_payments")
    .select("payment_id, remaining, released_from_charge_id")
    .eq("payment_id", paymentId)
    .maybeSingle();
  if (viewRes.error) {
    return { error: readFailedMessage("what is still unapplied on that payment", viewRes.error, { money: true }) };
  }
  const row = viewRes.data;
  const released = row?.released_from_charge_id != null;
  let allocated = 0;
  let unapplied = left;
  if (row) {
    const allocRes = await admin
      .from("park_payment_allocations").select("amount").eq("payment_id", paymentId).is("removed_at", null);
    if (allocRes.error) {
      return { error: readFailedMessage("what has already been put against bills", allocRes.error, { money: true }) };
    }
    allocated = allocatedTotal((allocRes.data ?? []).map((a) => ({ periodMonth: "", amount: Number(a.amount ?? 0) })));
    unapplied = { ...left, amount: Number(row.remaining ?? 0) };
  }
  // EVERY FIELD THE GUARD READS, NAMED — the `as never` cast that let
  // `returned_at` go unfetched for months is gone from this call.
  const forRefusal: RefundablePayment = {
    amount: (pay.amount as number | null) ?? null,
    fee_amount: (pay.fee_amount as number | null) ?? null,
    method: (pay.method as string | null) ?? null,
    reference: (pay.reference as string | null) ?? null,
    charge_id: (pay.charge_id as string | null) ?? null,
    released,
    kind: (pay.kind as string | null) ?? null,
    reversed_at: (pay.reversed_at as string | null) ?? null,
    returned_at: (pay.returned_at as string | null) ?? null,
  };
  const refusal = refundRefusal(forRefusal, unapplied);
  // Only the "exhausted" sentence is reworded — a return, a reversal or a
  // cash payment keeps its own more specific reason, which comes first.
  const exhausted = refusal === "All of that payment has already gone back.";
  return {
    ...unapplied,
    refusal:
      exhausted && allocated > 0
        ? `All of that payment is either against bills (${money(allocated)}) or already gone back — there is nothing unapplied to send.`
        : refusal,
    method: (pay.method as string | null) ?? null,
    released,
  };
...
export async function refundParkPayment(
  ...
  if (error) {
    ...
    if ((error as { code?: string }).code === "23505") {
      return { ok: true, signal: `${money(amount + feeAmount)} is on its way back.` };
    }
  ...
  revalidatePath("/park/today");
  revalidatePath("/park");
  // "THE BILL IS OUTSTANDING AGAIN" only when the bill is LIVE. A released
  // card row's bill is void (0169): the $70 going back came off what was
  // still on account, and the part month it settled is untouched.
  return { ok: true, signal: refundSignal(amount, feeAmount, Boolean(refRes.data?.charge_id) && !state.released, state.method) };
}
