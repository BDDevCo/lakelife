...
import { listAmenities, amenityToday } from "@/app/park/amenity-actions";

export default async function ParkAmenitiesPage() {
  if (!hasSupabaseEnv()) {
  ...
  // Lake time comes from the server. A client component with its own clock
  // decides "today has passed" in the browser's timezone, which is wrong for
  // anyone booking from a different one.
  const [rows, today] = await Promise.all([listAmenities(park.id), amenityToday()]);
