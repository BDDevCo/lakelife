...
-- ---------------------------------------------- 0100b: views take no writes --
-- All three views carry a join, so `information_schema.views.is_updatable` is
-- NO and Postgres refuses a write through them whatever the grant says. This
-- changes no behaviour; it exists so the standing audit query returns EXACTLY
-- the seven tables that legitimately need client writes. An audit with known
-- noise in it is an audit people stop reading.
