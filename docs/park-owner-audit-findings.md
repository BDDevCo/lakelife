...
**RESOLVED (0167, walk 18) — by applying the money, not by flagging the row.** The owner's third decision ("need it applied to the months if there is a prepay") settled it the other way round: money on account never sits beside an open bill. `park_payment_allocations` records which dollars paid which bill; the payment row never moves. One helper (`settleOnAccount`, src/lib/allocations.ts) puts a household's money on account against its OLDEST open bills, and it is called from every doorway money or a bill can enter by: the charge run the moment it raises a bill (`runCharges`), the recording door the moment money is keyed (`recordOnAccount`, `recordPayment` and therefore `confirmClaimCollected`), and the office's Apply by hand (`applyOnAccount`). So the Millers' 28 December cheque is against January the morning January is raised — Lot 7 reads 'Paid', the roll, ParkToday and the reminder run all read the same `paid_total`, and no letter is drafted. A cheque keyed on 4 January for a January bill that already exists is applied as it is keyed. The reminder plan needed no new skip: a bill settled from money on account is `paid`. What remains deliberate: a bill with NO money on account is still late, and an allocation can be taken back off a bill only by the office, with a reason (`unapplyAllocation`), never by anything automatic.
